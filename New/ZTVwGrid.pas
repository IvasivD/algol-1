...
uses
  ...
  ZTView, Grids, SvtDBGrids, ActnList, Menus, Db, DBTables, StdCtrls, Buttons,
  ExtCtrls, ImgList, Variants, ComObj, IBCustomDataSet, IBQuery;
...
begin
  ...
  try
    Close;
    SortStr := WorkDataSetSQL[WorkDataSetSQL.Count - 1];//SQL[SQL.Count - 1];
    try
//      SQL[SQL.Count - 1] := 'order by ' + ColName;
//      Prepare;
      WorkDataSetSQL[WorkDataSetSQL.Count - 1] := 'order by ' + ColName;
      Open;
...
//      Prepare;
      WorkDataSetSQL[WorkDataSetSQL.Count - 1] := SortStr;
      Open;
      FGridSortColumn := -1;
    end;
    if (GridUniqueColumn <> nil) and (Id <> 0)
      and (Column.Field.DataSet = WorkDataSet {DetailListQuery})
...
begin
  Result := CountLabelCaption(
    {DetailListQuery}WorkDataSet.RecordCount, SDataModQueryRecordCountOne,
...
begin
  if Print then
    ...
    RepCols := PersonListDBGrid.Columns.Count + 1;
    ...
    J := 0;
//    with DetailListQuery do
    with PersonListDBGrid.DataSource.DataSet do
    begin
      Screen.Cursor := crHourGlass;
      AfterScroll := nil;
      DisableControls;
...
begin
  if Field.FieldName = 'CalcMemo' then Field := WorkDataSet.FieldByName('Memo');//DetailListQuery.FieldByName('Memo');
...
begin
  ...
  begin
    ...
    try
      DisableControls;
      if WorkDataSet is TQuery then TQuery(WorkDataSet).FetchAll; 
      if WorkDataSet is TIBQuery then TIBQuery(WorkDataSet).FetchAll;       
      First;     
