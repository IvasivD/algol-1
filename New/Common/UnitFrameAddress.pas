...
type
  ...
  TParamAddres = record
    ...
    FieldNameId_City_adm_unit:string;
  ...
  TFrAddress = class(TFrame)
    ...
    EdDistrictCity: TcxTextEdit;
    LbCityDistrict: TLabel;
  ...
  private
    ...
    function GetIdCityAdmUnit: variant;
    function GetIdCountry: variant;
    function GetIdStreetKind: variant;
  public

    { Public declarations }
    function  CheckError(ShowErrorMes:Boolean):Boolean;
    procedure SaveDataControls( isNew:Boolean; ParamAddres:TParamAddres; AutoPost:boolean=false );
    ...
    procedure Deactive;
    property ID_Country:variant read GetIdCountry ;
    property ID_CityAdmUnit:variant read GetIdCityAdmUnit;
    Property ID_Str_kind:variant read GetIdStreetKind;
...
begin
  ...
  begin
    result.DB := UnitIBQuery.CreateDataSet( ConParam.PathDB,ConParam.UID,ConParam.Password, nil, Sql, Active );
...
begin
  try
    ...
    EdDistrictCity.Visible := ParamAddres.FieldNameDistrictCity <>'';
    LbCityDistrict.Visible := ParamAddres.FieldNameDistrictCity <>'';
    
    if not isNew then
    begin
      ExtLupComboboxCountry.EditValue := aElem.DB.FieldByName( ParamAddres.FieldNameCountryId ).Value;
      FrameSearchCityAddres.SetValue( aElem.DB.FieldByName( ParamAddres.FieldNameId_City_adm_unit ).Value );
      ...
      if ParamAddres.FieldNameDistrictCity<>'' then
        EdDistrictCity.Text := aElem.DB.FieldByName( ParamAddres.FieldNameDistrictCity ).asString;
...
procedure TFrAddress.SaveDataControls( isNew:Boolean; ParamAddres:TParamAddres; AutoPost:boolean=false );
var aElem:TDBShema;
begin
  aElem :=ParamAddres.ElemAddres;
  ...
  aElem.DB.FieldByName( ParamAddres.FieldNameId_City_adm_unit ).Value := FrameSearchCityAddres.EditValue;
  ...
  if AutoPost then aElem.DB.Post;
end;
...
function TFrAddress.GetIdCountry: variant;
begin
  result := ExtLupComboboxCountry.EditValue;
end;

function TFrAddress.GetIdCityAdmUnit: variant;
begin
  result := FrameSearchCityAddres.EditValue;
end;

function TFrAddress.GetIdStreetKind: variant;
begin
  result := CmBoxStreet.EditValue;
end;
