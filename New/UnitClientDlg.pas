...
var EnterpValue:Integer;
    EnterprCondition:string;
begin
  if ClientRadioGroup.ItemIndex = 1 then EnterprCondition := '='
  else EnterprCondition := '<>';
  //*****************************
  if not IsVendorCheckBox.Checked then
    FElem.SetFilterOnView( [ 'IsEnterpr' ], [ 1 ], [EnterprCondition] )
  else
    FElem.SetFilterOnView( [ 'IsEnterpr', 'IsVendor' ], [ 1, 1 ], [ EnterprCondition ,'='],fboAnd );
  //*****************************
  SearchEdit.Text := '';
...
begin
  if FElem<> nil then
    FElem.AddFocusedRecordChanged( nil );
end;
