...
function RenameExtToPdf(const aFileName: string): string;
...
function GetDocStoreFolder(const StorePath: String; SubDir: array of Integer;
...
function MakeDocStoreFileName(var StorePath, DocName: String; 
  SubDir: array of Integer; const TypeFolder: Integer = 0; IsMail: Boolean = True): Boolean;
function GetDocumentStoreFolderArhiv(const StoreFolder: string; SubDir: array of Integer;
  const TypeFolder: Integer = 0): string;
...
function GetDocumentStoreFolder(const StoreFolder: string; SubDir: array of Integer; {old GetOrderDocumentStoreFolder}
  const TypeFolder: Integer = 0): string;
...
begin
  if IsStrANumber(Value) then Result := StrToFloat(Value)
  else Result := Excpt;
...
begin
  if VarIsNull(V) then Result := False
    ...
    S := SafeVarToStr(V);
    Result := (Length(S) > 0) and (S[1] in ['T', 't', 'Y', 'y', '1']);
  end;
end;
...
function SafeVarToInt(V: Variant): Integer;
begin
  if VarIsNull(V) or not VarIsNumeric(V) then Result := 0 else Result := V;
end;
...
function RenameExtToPdf(const aFileName: string): string;
begin 
  Result := StringReplace(aFileName, ExtractFileExt(aFileName), '.pdf', []);
end;
...
var RefOrd: Boolean;
    I: Integer;
begin
  RefOrd := Length(SubDir) > 0;
  Result := IncludeTrailingPathDelimiter(StorePath) + sFldrDocs + 
    PathDelim + sFldr[TypeFolder] + PathDelim;
  if RefOrd then
    for I := 0 to Length(SubDir) - 1 do
      Result := Result + IntToStr(SubDir[I]) + PathDelim;
end;
...
begin
  RefOrd := Length(SubDir) > 0;
  Fldr := GetDocumentStoreFolder{GetDocStoreFolder}(StorePath, SubDir, TypeFolder);
      ...
      StringReplace(
        StringReplace(
          StringReplace(DocName, '"', '_', [rfReplaceAll]), 
            '.', '_', [rfReplaceAll]),
              ':', '_', [rfReplaceAll]) +
               FormatDateTime(sTimeMrk, Now);
    if IsMail then DocName := DocName + sMailExt;
  end;
end;
...
begin
  ...
  Result := GetDocStoreFolder(SFolter, SubDir, TypeFolder);
end;

function GetDocumentStoreFolder(const StoreFolder: string; SubDir: array of Integer;
  const TypeFolder: Integer = 0): string;
begin
  Result := GetDocumentStoreFolderArhiv(StoreFolder, SubDir, TypeFolder);
  if (Result = EmptyStr) or not DirectoryExists(Result)
  then Result := GetDocStoreFolder(StoreFolder, SubDir, TypeFolder);
