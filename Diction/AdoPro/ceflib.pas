...
procedure CefFreeLibDefault;
procedure CefLoadLibDefault;
procedure CefLoadLib(const Cache: ustring = '';
  const UserDataPath: ustring = ''; const UserAgent: ustring = '';
...
var
  ...
  CefCache: ustring = '';
  CefUserDataPath: ustring = 'GHROME\';
...
begin
  if LibHandle = 0 then
    CefLoadLib(CefCache, CefUserDataPath, CefUserAgent, CefProductVersion, CefLocale, CefLogFile,
...
procedure CefLoadLib(const Cache, UserDataPath, UserAgent, ProductVersion, Locale, LogFile, ExtraPluginPaths: ustring;
...
{$ENDIF}
    if UserDataPath<>'' then
      PathToLib := ExtractFilePath(Application.ExeName)+UserDataPath
    else PathToLib :='';

    LibHandle := LoadLibrary(PChar(PathToLib+CefLibrary));
    if LibHandle = 0 then
      RaiseLastOSError;
...
begin
  ...
  begin
    cef_shutdown;
    if LibHandle > HINSTANCE_ERROR then FreeLibrary(LibHandle);
    LibHandle := 0;
  end;
end;
