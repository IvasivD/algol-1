...
uses
  ...
  cxGridCustomView, cxGridCustomTableView, cxGridTableView, TourDbUn,UnitFrameAddress,AddrDlg,
  cxGridDBTableView, cxGrid, ExtCtrls, Buttons,TourStrMag,EmailFrm, OptSet, Core,UnitProc,
  OleServer, WordXP, MailSend_Frm,TourConsts,TourCmnlIf, UnitEditPhone, UnitExportWord,
  WinFaxSendForm,VDlgs, SKYPE4COMLib_TLB,OrderUn,UnitContractEditForm,UnitIBQuery,UnitADOQuery,IBQuery,ADODB,
  ...
  cxCalendar, cxImage, cxDBExtLookupComboBox, cxImageComboBox, cxBlobEdit,
  cxDBLookupComboBox, cxButtonEdit, dxmdaset, ImgList,ServUn,  IdHTTP, ShellAPI, ShFolder,MaskUtils,
  UnitFrameCompanyPayState, cxMaskEdit;
...
const
  ...
  sIndvdPar = 'Indvd'; 
  sErrandPar = 'Errand';
  arrNamePar: array [0..4] of string = (
    sNamePar, sEntrpPar, sBsnmPar, sIndvdPar, sErrandPar);
...
type
  ...
  TFrameComRequisites = class(TFrame)
    ...
    OfficeQryCITYKIND: TSmallintField;
    ...
    ViewOfficesCITYKIND: TcxGridDBColumn;
    ...
    ViewOfficesCalcAddres: TcxGridDBColumn;
    Panel10: TPanel;
    Panel11: TPanel;
    Panel12: TPanel;
  ...
  public
    ...
    procedure IntitComRquisitesFrame( ParamFrame:TParamComRequisitesFrame; {aGetPartnersDocsStoreFolder:WideString;} CanUpdate :Boolean; aCurrManager,aLevMngPermis:Integer; aCurManagerName:Widestring  );
    procedure DestroyFrame;
    procedure ReQueryAllDbOffice;
    procedure SetManager(CurrManager,FLevMngPermis:Integer; FCurManagerName:Widestring; CanUpdate:Boolean);
  protected
...
begin
  ...
  with ElemHotel.DB.FieldByName('ID_CITY_ADM_UNIT') do if IsNull then CityId := NewCityId else CityId := Value;
  ElemHotel.DB.Post;
  ...
  ElemOffice.DB.FieldByName('ID_CITY_ADM_UNIT').Value := CityId;
  ElemOffice.DB.Post;
end;
...
procedure TFrameComRequisites.IntitComRquisitesFrame( ParamFrame:TParamComRequisitesFrame; {aGetPartnersDocsStoreFolder:WideString;} CanUpdate :Boolean; aCurrManager,aLevMngPermis:Integer; aCurManagerName:Widestring );
...
begin
  ...
  PrepareCompCount;
  SetManager( aCurrManager, aLevMngPermis, aCurManagerName, CanUpdate );
...
begin
  ...
  result.ParamAddres.FieldNameCityId    :='ID_CITY_ADM_UNIT';
...
begin
  ...
  //**********************
  butAddAddres.Enabled := (FGetCountRecCompanyGridDB <> 0) and (FCurManager > 0);
  butEditAddres.Enabled := ( not Elem_OfficeQry.DB.IsEmpty ) and (FCurManager > 0);
  butDelAddres.Enabled := ( not Elem_OfficeQry.DB.IsEmpty ) and (FCurManager > 0);
  //**********************
  butAddNote.Enabled :=  (FGetCountRecCompanyGridDB <> 0)and(FCurManager > 0);
  butPreviewNote.Enabled := (TcxGridDBTableView( GridNotes.Levels[0].GridView).ViewData.RowCount <> 0) and (FCurManager > 0);
  ContractAddNewBtn.Enabled := (FGetCountRecCompanyGridDB <> 0)and(FCurManager > 0);
  ContractEditBtn.Enabled := ( Elem_ContractQry.DBRecordCount <> 0 ) and (FCurManager > 0);
  ...
  //**********************
  butAddKindBranch.Enabled := (FGetCountRecCompanyGridDB <> 0)and(FCurManager > 0);
  butDelKindBranch.Enabled := ( TcxGridDBTableView( GridKindBranch.Levels[0].GridView ).ViewData.RowCount <> 0 )  and (FCurManager > 0);
  //**********************
  butAddPersons.Enabled  := (FElemCompanyQry.DBRecordCount <> 0) and( FCurManager > 0 );
  butEditPersons.Enabled := (Elem_PersonQry.View.ViewData.RowCount <> 0) and( FCurManager > 0 );
  butDelPersons.Enabled  := (Elem_PersonQry.View.ViewData.RowCount <> 0) and( FCurManager > 0 );
  //**********************
  butAddCompanyComunication.Enabled :=  (FElemCompanyQry.DBRecordCount <> 0) and( FCurManager > 0 );
  butEditCompanyComunication.Enabled := ( TcxGridDBTableView(GridComunicationCompany.Levels[0].GridView ).ViewData.RowCount <> 0 ) and (FCurManager > 0);
  butDelCompanyComunication.Enabled :=  ( TcxGridDBTableView(GridComunicationCompany.Levels[0].GridView ).ViewData.RowCount <> 0 ) and (FCurManager > 0);
  ...
  //**********************
  butAddOfficeComunication.Enabled :=  (TcxGridDBTableView(GridOffices.Levels[0].GridView).ViewData.RowCount <> 0) and (FCurManager > 0);
  butEditOfficeComunication.Enabled:=  (TcxGridDBTableView(GridComunicationCompOffice.Levels[0].GridView).ViewData.RowCount <> 0) and (FCurManager > 0);
  butDelOfficeComunication.Enabled :=  (TcxGridDBTableView(GridComunicationCompOffice.Levels[0].GridView).ViewData.RowCount <> 0) and (FCurManager > 0);
  ...
  //**********************
  butAddPersonsComunication.Enabled := (Elem_PersonQry.View.ViewData.RowCount <> 0) and( FCurManager > 0 );
  buEditPersonsComunication.Enabled := (Elem_Comunication_PersonQry.View.ViewData.RowCount <> 0) and( FCurManager > 0 );
  butDelPersonsComunication.Enabled := (Elem_Comunication_PersonQry.View.ViewData.RowCount <> 0) and( FCurManager > 0 );
  butCallToLinklPersonsComunication.Enabled := buEditPersonsComunication.Enabled;
  //**********************
  butAddCopmPersonComunication.Enabled := ( Elem_PersonQry.View.ViewData.RowCount <> 0) and( FCurManager > 0 );
  butEditCopmPersonComunication.Enabled:=  ( TcxGridDBTableView(GridComunicationCompPerson.Levels[0].GridView ).ViewData.RowCount <> 0 ) and( FCurManager > 0 );
  butDelCopmPersonComunication.Enabled :=  ( TcxGridDBTableView(GridComunicationCompPerson.Levels[0].GridView ).ViewData.RowCount <> 0 ) and( FCurManager > 0 );
  ...
  //**********************
  butAddAccount.Enabled :=  (FElemCompanyQry.DBRecordCount <> 0) and( FCurManager > 0 );
  butEditAccount.Enabled := ( Elem_AccountQry.View.ViewData.RowCount <> 0 ) and( FCurManager > 0 );
  butDelAccount.Enabled := ( Elem_AccountQry.View.ViewData.RowCount <> 0 ) and( FCurManager > 0 );
  //**********************
...
//  ParamContract.ElemSelCntrTemplQry :=Elem_SelCntrTemplQry;
  ...
  if ( CheckCompanyData ) and ( ExecuteContractEditForm( ParamContract, self.FCurManager, FLevMngPermis, self.FCurManagerName, ActionTag, True, self.FCanUpdate ) ) then
  begin
...
begin
  ...
  try
    ...
    begin
      aDB := UnitIBQuery.CreateDataSet( DBParam.PathDB, DBParam.UID, DBParam.Password, GetSqlContractTemplatesIB(TypeTempl), False );
...
procedure TFrameComRequisites.SetManager( CurrManager,
  FLevMngPermis: Integer; FCurManagerName: Widestring; CanUpdate:Boolean );
begin
